...
- **email** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ SMTP settings consumed by `automation/emailer.py`.

- **risk_management** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Maximum positions, individual/portfolio stops, and sector limits used by `portfolio.position_sizer.PositionSizer` and `portfolio.health.PortfolioHealthMonitor`.

- **strategy_weights** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Default allocations for combined backtests and reporting weights.

- **data_sources** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Caching controls for Yahoo downlinks.
...
- **universe_criteria** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Market-cap, volume, float, spread, and sector filters enforced by `universe/builder.py`.

- **storage** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Output directories for cached prices, signal archives, and portfolio snapshots.
...
- `scan` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Run the universe screening pipeline.

- `backtest` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Backtest one or more strategies.

- `report` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Inspect previously generated CSV reports.

- `health` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Evaluate drawdown and sector health of a portfolio snapshot.

- `notebook` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Copy the prebuilt analysis notebook to a destination workspace.
...
- Alpha Vantage overview plus earnings feeds are blended into valuation, profitability, leverage, and growth metrics that feed the fundamentals card.
- Each symbol receives a composite fundamentals score with tooltip guidance and timestamps so you know when data was last refreshed.
...
1. **Seed list** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Start from `data/universe/seed_candidates.csv` or an explicit ticker list (`--symbols`).
...
3. **Filters** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Applies market cap, volume, bid/ask spread, float, sector, and exchange screens as defined in config.

4. **Russell 2000 merge** - Use `--include-russell` to append the bundled constituents from `data/universe/russell_2000.csv`.

4. **Persistence** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Saves accepted names to `storage.universe_dir` unless `--no-persist` is supplied.

5. **Diagnostics** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Reports skipped symbols for missing data or API errors.
...
- `backtesting.engine.BacktestingEngine` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Runs each strategy over a price frame (CSV or Yahoo download via `--symbol`).

- `backtesting.runner.StrategyBacktestRunner` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Manages per-strategy runs with a shared position-sizer.
...
- `portfolio.position_sizer.PositionSizer` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Respects risk config (max positions, sector caps, stop levels) when allocating capital to generated signals.

- `reports.performance.build_performance_report` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Calculates CAGR, Sharpe, drawdown metrics.

- `reports.attribution.compute_attribution` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Weights returns by strategy allocations for LP-style contributions.

- `backtesting.combiner.combine_equity_curves` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Produces a weighted composite equity curve.
...
- `portfolio.equity_curve.load_equity_curve` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Reads date-indexed equity history.

- `portfolio.health.PortfolioHealthMonitor` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ Aggregates drawdown alerts (via `portfolio.alerts.DrawdownAlertManager`) and sector breaches (via `portfolio.sector_monitor.detect_sector_breaches`).
...
4. Opening a watchlist item routes you to /symbols/:ticker, refreshes the live price/strategy/fundamentals analyses, and writes the refreshed snapshot back to the database so stale entries are automatically replaced.
